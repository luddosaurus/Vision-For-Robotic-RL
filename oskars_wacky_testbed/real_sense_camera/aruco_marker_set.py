...
class MarkerSet(object):
    ...
    def calc_distance_from_each(self):
        ...
        for i in range(0, len(self.ids)):
            ...
            rvec_i, tvec_i, markerPoints_i = cv2.aruco.estimatePoseSingleMarkers(self.corners[i], self.marker_size, self.matrix_coefficients, self.distortion_coefficients)
            c_i = self._find_center(self.corners[i].reshape((4,2)))
            
            for j in range(i+1, len(self.ids)):
                rvec_j, tvec_j, markerPoints_j = cv2.aruco.estimatePoseSingleMarkers(self.corners[j], self.marker_size, self.matrix_coefficients, self.distortion_coefficients)

                distance = self.calculate_distance_between_two_markers(tvec_i, tvec_j)
                # print(f'{i} - {j}: {diff}')

                c_j = self._find_center(self.corners[j].reshape((4,2)))
                cv2.line(self.image, c_i, c_j, (147, 20, 255), 2)

                cv2.putText(self.image, f"{distance:.2f}", (int(abs(c_i[0] + c_j[0])/2), int(abs(c_i[1] + c_j[1])/2)),cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0),2,cv2.LINE_AA,)
                cv2.drawFrameAxes(self.image, self.matrix_coefficients, self.distortion_coefficients, rvec_j, tvec_j, 3.4/2)
            
            cv2.drawFrameAxes(self.image, self.matrix_coefficients, self.distortion_coefficients, rvec_i, tvec_i, 3.4/2)
    ...
    def calc_distance_from_each_area_version(self):
        ...
        if len(self.corners) == 3:
            area = self.calculate_triangle_area(self.distance_dict_triangle["0-1"], self.distance_dict_triangle["0-2"], self.distance_dict_triangle["1-2"]) 
            cv2.putText(self.image, f"Area: {area:.2f}", (int(abs(c_i[0] + c_j[0])/2), int(abs(c_i[1] + c_j[1])/2)),cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0),2,cv2.LINE_AA,)
        return self.image
    ...
    def _find_center(self, mark_corners):
