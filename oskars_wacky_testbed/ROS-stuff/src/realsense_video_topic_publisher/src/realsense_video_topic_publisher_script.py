...
def main():
    ...
    pub = rospy.Publisher('/realsense_camera', Image, queue_size=10)
    rate = rospy.Rate(1)


    print("f - Publish continuously\ng - Don't publish continuously\ns - Publish single image while not publishing continuously\nq - Quit")
    ...
    while not rospy.is_shutdown(): 
        try:    
            while True:
                ...
                # Show images
                try:
                    img_message = cv_bridge.cv2_to_imgmsg(color_image, encoding="passthrough")
